...
create_data = function(observation_num, start_year, time_type, time_params, beta_1, id_type, id_params, beta_2) {
  all_years = seq(start_year, 2020, by = 2)
  data_raw = data.frame()
...
run_on_cluster(
  ...
  first = {
    ...
    sim %<>% set_levels(
      # observation_num = c(1000, 2000),
      observation_num = 1000,
      start_years = 1990,
      cvr_list = list(
        "Norm_Exp" = list(time_type = "Normal", time_params = c(4, 5, 2, 1), time_beta = 0.7, id_type = "Exponential", id_params = c(log(sqrt(3.2)), log(sqrt(5/4))), id_beta = 3),
        "Pois_Pois" = list(time_type = "Poisson", time_params = 8, time_beta = 2, id_type = "Poisson", id_params = 2, id_beta = 1.5),
        "Uni_Gam" = list(time_type = "Uniform", time_params = c(1, 3), time_beta = 11, id_type = "Gamma", id_params = c(3, 2), id_beta = 12)
      )
    )
    ...
    sim %<>% set_script(function() {
      data = create_data(L$observation_num, L$start_year, L$cvr_list$time_type, L$cvr_list$time_params, L$cvr_list$time_beta, L$cvr_list$id_type, L$cvr_list$id_params, L$cvr_list$id_beta)$data
  data_raw = create_data(L$observation_num, L$start_year, L$cvr_list$time_type, L$cvr_list$time_params, L$cvr_list$time_beta, L$cvr_list$id_type, L$cvr_list$id_params, L$cvr_list$id_beta)$raw
      ...
      print(mean(mean_x1_2lpmm))
      
      return(list(
        "x1_true" = mean(data_raw$x1),
        "x1_2lpmm" = mean(mean_x1_2lpmm),
        "x1_pmm" = mean(mean_x1_pmm),
        "2l_pctg" = (x1_2lpmm - x1_true) / x1_true * 100,
        "1l_pctg" = (x1_pmm - x1_true) / x1_true * 100,
        ".complex" = list(
          "data_raw" = data_raw,
          "data" = data
        )
  ...
  last = {
    ...
    bias_Q_pct = sim %>% SimEngine::summarize(
      list(stat = "mean", x = "2l_pctg", name = "bias_x1_2lpmm"),
      list(stat = "mean", x = "1l_pctg", name = "bias_x1_pmm")
    )
  },
