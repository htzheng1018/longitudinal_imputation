...
create data
```{r}
create_data = function(obervation_num, all_years, time_type, time_params, beta_1, id_type, id_params, beta_2) {
  data_raw = data.frame()
...
data = create_data(obervation_num, all_years, "Poisson", 8, 2, "Exponential", c(log(sqrt(3.2)), log(sqrt(5/4))), 3)$raw



for (i in 1:5) {
  print(mean(complete(imp.use, i)$x1))
}
...
run_on_cluster(
  ...
  first = {
    ...
    sim %<>% set_script(function() {
      data = create_data(L$obervation_num, L$all_years, L$time_cvr$distr_type, L$time_cvr$distr_params, L$time_cvr$beta, L$id_cvr$distr_type, L$id_cvr$distr_params, L$id_cvr$beta)$data
      data_raw = create_data(L$obervation_num, L$all_years, L$time_cvr$distr_type, L$time_cvr$distr_params, L$time_cvr$beta, L$id_cvr$distr_type, L$id_cvr$distr_params, L$id_cvr$beta)$raw
      ...
      # imputation process
      imp.use2l = mice(data_use, pred = pred, meth = meth2l, m = 5, maxit = 5, seed = 1018, printFlag = F)
